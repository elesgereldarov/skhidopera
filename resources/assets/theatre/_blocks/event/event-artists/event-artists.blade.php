    ...
    <div class="row event-artists__list">
        @foreach($groupActorDates as $actorDates)
            ...
            @foreach ($actorDates as $actorDate)
                @set($actorDatesList[], \Illuminate\Support\Carbon::parse($actorDate->date->date)->format('Y-m-d'))
            @endforeach
            <div class="col-sm-4 event-artists__item d-flex align-items-stretch"
                 data-date="{{ implode(',', $actorDatesList) }}"
                 data-event-artist>
                @include('pages.theatre._blocks.event.event-artist.event-artist', ['actorDate' => $actorDates->first(), 'actorDates' => $actorDates])
            </div>
        @endforeach
    </div>
</section>
